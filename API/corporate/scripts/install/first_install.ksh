...
then
	# On quitte le script
	echo "Syntaxe : ./first_install.ksh <path_api> <server> <rootpwd>"
	exit 1
fi
...
if [[ ${?} -ne 0 ]]
then
	# Oui donc on quitte le script
  	echo "An API installation already exist."
	#exit 1
fi 
...
# Creation du repertoire versions et du repertoire contenant l'API
ssh api@${server} "mkdir /home/api/versions; mkdir /home/api/versions/${api_name}"
...
ssh api@${server} "ln -s /home/api/versions/${api_name} /home/api/APIX2P"
